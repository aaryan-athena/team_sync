...
async function startLiveFeed() {
  ...
  try {
    ...
    liveState.ws = new WebSocket(`${wsProto}//${location.host}/live/ws?mode=${liveState.mode}&show_feed=${liveState.showFeed}&thresholds=${thresholdsJSON}`);
    ...
    liveState.ws.onmessage = event => {
      ...
      if (msg.type === 'frame') {
        ...
        // No-feed mode: tiny JSON payload, no JPEG decode needed
...
function updateLiveUI() {
  ...
  // Grey out the toggle while a session is running
...
function toggleFeedDisplay() {
  if (liveState.active) return;  // locked while a session is running
