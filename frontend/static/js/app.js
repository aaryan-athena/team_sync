...
function renderLiveCanvas() {
  ...
  const canvas = $('liveCanvas'), ctx = canvas.getContext('2d'), video = $('liveVideo');
  const vw = video.videoWidth || 640, vh = video.videoHeight || 480;
  if (canvas.width !== vw)  canvas.width  = vw;
  if (canvas.height !== vh) canvas.height = vh;
  if (video.readyState >= 2) ctx.drawImage(video, 0, 0, vw, vh);
  const fw = liveState.lastFrameSize.w || vw, fh = liveState.lastFrameSize.h || vh;
...
function drawDetections(ctx, detections, sx, sy) {
  ...
  for (const d of detections) {
    const [x1, y1, x2, y2] = d.box, color = DETECT_COLORS[d.cls] || '#fff';
...
function stopLiveFeed() {
  ...
  if (wasActive && liveState.sessionStart) {
    ...
    liveState.sessionData = {
      ...
      duration_seconds: dur, shots: liveState.stats?.shots || 0,
      baskets: liveState.stats?.baskets || 0, accuracy: liveState.stats?.accuracy || 0,
      persons: liveState.stats?.persons || 0, mode: liveState.mode
    };
...
function updateLiveUI() {
  ...
  setVisible('liveCanvas', isLive); setVisible('livePlaceholder', !isLive);
  setVisible('liveDot', isLive); setVisible('btnStartLive', !isActive);
  setVisible('btnStopLive', isActive); setVisible('liveStatsSection', isLive);
